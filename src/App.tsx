    ...
    <div className={`shell ${chatOpen ? 'chat-expanded' : ''}`}>
      <header className="header">
        ...
        <div className="header-actions">
          <button className={`small-action ${tab === 'llmIntegration' ? 'active' : ''}`} onClick={() => setTab('llmIntegration')}>🤖 LLM API</button>
          <button className="small-action" onClick={downloadResumePdf}>📥 Download PDF</button>
          <button className={`small-action ${chatOpen ? 'active' : ''}`} onClick={() => setChatOpen((v) => !v)}>{chatOpen ? '🧩 Hide Chat' : '💬 Show Chat'}</button>
        </div>
      </header>
      <div className="app-layout">
        <div className="main-content">
          <nav className="tabs"><button className={tab === 'resume' ? 'active' : ''} onClick={() => setTab('resume')}>📝 Resume Builder</button><button className={tab === 'coverLetter' ? 'active' : ''} onClick={() => setTab('coverLetter')}>✉️ Cover Letter</button><button className={tab === 'history' ? 'active' : ''} onClick={() => setTab('history')}>📚 Submission History</button></nav>
          ...
          {tab === 'resume' && <>
            ...
            <section className="workspace">
              <aside className="skills-panel">
                <div className="panel-header"><h3>🧠 Experience Repository</h3><button className="small-action" onClick={() => setSectionsCollapsed((prev) => !prev)}>{sectionsCollapsed ? '⬇️ Expand Filters' : '⬆️ Collapse Filters'}</button></div>
                {!sectionsCollapsed && <div className="filters-line"><select value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)}>{companies.map((company) => <option key={company} value={company}>{company}</option>)}</select><select value={skillFilter} onChange={(e) => setSkillFilter(e.target.value)}>{skills.map((skill) => <option key={skill} value={skill}>{skill}</option>)}</select><button onClick={applySelectedExperience}>✅ Apply Selected</button></div>}
                <div className="experience-list">{filteredExperience.map((item) => <label key={item.id}><span><input type="checkbox" checked={item.selected} onChange={() => toggleExperience(item.id)} /> {item.text}</span><small>{item.company} · {item.skillTags.join(', ')}</small></label>)}</div>
              </aside>

              <main className={`resume-preview ${template.toLowerCase()} ${previewFullscreen ? 'is-fullscreen' : ''}`} ref={previewRef}>
                <div className="preview-toolbar"><button onClick={() => setZoom((prev) => Math.max(0.7, prev - 0.1))}>➖</button><button onClick={() => setZoom((prev) => Math.min(1.5, prev + 0.1))}>➕</button><button onClick={() => setEditMode((prev) => !prev)}>{editMode ? '✅ Done' : '✍️ Edit'}</button><button onClick={() => setSectionPickerOpen((prev) => !prev)}>{sectionPickerOpen ? '📂 Hide Sections' : '📁 Sections'}</button><button onClick={togglePreviewFullscreen}>{previewFullscreen ? '🗗 Exit Fullscreen' : '🗖 Fullscreen'}</button><span className="toolbar-right">Zoom {Math.round(zoom * 100)}%</span></div>

                {sectionPickerOpen && <div className="section-picker"><div className="section-picker-header top-section-row"><strong>Section controls</strong><button onClick={() => setSectionsCollapsed((prev) => !prev)}>{sectionsCollapsed ? '⬇️ Expand List' : '⬆️ Collapse List'}</button></div>{!sectionsCollapsed && sections.map((section) => <div key={section.id} className="section-picker-row"><button onClick={() => toggleSection(section.id)}>{section.visible ? '👁️ Hide' : '👁️ Show'}</button><input value={section.label} onChange={(e) => updateSectionLabel(section.id, e.target.value)} /></div>)}</div>}

                <div className="preview-frame"><div className="preview-content" style={{ transform: `scale(${zoom})` }}>
                  {sectionMap.header.visible && <><h2>{resume.fullName || 'Your Name'}</h2><p className="preview-text">{resume.email || 'email@example.com'} · {resume.phone || '(000) 000-0000'} · {resume.location}</p><p className="preview-text">Target: {resume.targetRole || 'Role'}</p></>}
                  ...
                  {sectionMap.skills.visible && <><h4>{sectionMap.skills.label}</h4>{editMode ? <input value={resume.keySkills.join(', ')} onChange={(e) => setResume((prev) => ({ ...prev, keySkills: e.target.value.split(',').map((x) => x.trim()).filter(Boolean) }))} /> : <p className="preview-text">{resume.keySkills.join(' • ')}</p>}</>}
        ...
        <aside className={`chat-sidebar ${chatOpen ? 'open' : 'collapsed'}`}>
          <section className="chat-box"><h4>🤝 Chat Agent</h4><div className="chat-feed">{chatMessages.map((msg, idx) => <p key={`${msg}-${idx}`}>{msg}</p>)}</div><textarea placeholder="Answer missing-info questions here" value={chatInput} onChange={(e) => setChatInput(e.target.value)} /><button className="primary" onClick={submitGapAnswer}>📤 Send to Refiner</button></section>
        </aside>
      </div>
    </div>
  )
