...
function App() {
  ...
  const sectionMap = useMemo(
    () =>
      ...
      ) as Record<SectionId, ResumeSection>,
    [sections],
  );
  ...
  async function submitGapAnswer() {
    ...
    setResume((prev) => ({
      ...prev,
      summary: String(
        (patchedDraft.summary as string | undefined) || prev.summary,
      ),
      keySkills: (
  ...
  function downloadResumePdf() {
    ...
    y += 8;
    doc.setFontSize(11);
    doc.text(`${resume.email} | ${resume.phone} | ${resume.location}`, 14, y);
    y += 8;
    doc.text(`Target Role: ${resume.targetRole}`, 14, y);
    y += 8;
    doc.text(resume.summary, 14, y, { maxWidth: 180 });
    y += 16;
    ...
    resume.selectedExperience.forEach((item) => {
      doc.text(`• ${item.text}`, 14, y, { maxWidth: 180 });
      y += 7;
      if (y > 275) {
        doc.addPage();
        y = 20;
                      ...
                      {sectionMap.header.visible && (
                        <>
                          ...
                          <p className="preview-text">
                            ...
                            {resume.phone || "(000) 000-0000"} ·{" "}
                            {resume.location}
                          </p>
                          <p className="preview-text">
                            Target: {resume.targetRole || "Role"}
                          </p>
                        </>
                      )}
                            ...
                            <div className="bullet-row" key={item.id}>
                                ...
                                <textarea
                                  ...
                                  onChange={(e) =>
                                    setResume((prev) => ({
                                      ...prev,
                                      selectedExperience:
                                        prev.selectedExperience.map((entry) =>
                                          entry.id === item.id
                              ...
                              {editMode && (
                                <div className="move-controls">
                                  <button onClick={() => moveBullet(idx, "up")}>
                                    ↑
                                  </button>
                                  <button
